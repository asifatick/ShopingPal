   ...
   public sealed class WebSecurity
    {
        ...
        public static MembershipUser CreateFBUser(string username, FacebookUserModel fbUser, string email, string passwordQuestion, string passwordAnswer, bool isApproved, object providerUserKey, out MembershipCreateStatus status)
        {
           return CodeFirstMembershipProvider.CreateFBUser(username, fbUser, email, passwordQuestion, passwordAnswer, isApproved, providerUserKey, out status);        
        }
        ...
        public static bool TryGetFBUser(string email, string fbid)
        {
            using (DataContext Context = new DataContext())
            {
                if (Context.Users.Where(Usr => Usr.Email == email || Usr.FaceBookID == fbid).Any())
                {
